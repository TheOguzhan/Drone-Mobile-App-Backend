...
type OrderQuery struct {
	...
	withCarrierDrone   *DroneQuery
	withUserOrder      *UserQuery
	withOrderWarehouse *WarehouseQuery
	withOrderAddress   *AddressQuery
	withOrderProduct   *ProductQuery
	withFKs            bool
...
// QueryCarrierDrone chains the current query on the "carrier_drone" edge.
func (oq *OrderQuery) QueryCarrierDrone() *DroneQuery {
	query := (&DroneClient{config: oq.config}).Query()
...
// QueryOrderWarehouse chains the current query on the "order_warehouse" edge.
func (oq *OrderQuery) QueryOrderWarehouse() *WarehouseQuery {
	query := (&WarehouseClient{config: oq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := oq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// QueryOrderProduct chains the current query on the "order_product" edge.
func (oq *OrderQuery) QueryOrderProduct() *ProductQuery {
...
func (oq *OrderQuery) Clone() *OrderQuery {
	...
	return &OrderQuery{
		...
		withCarrierDrone:   oq.withCarrierDrone.Clone(),
		withUserOrder:      oq.withUserOrder.Clone(),
		withOrderWarehouse: oq.withOrderWarehouse.Clone(),
		withOrderAddress:   oq.withOrderAddress.Clone(),
		withOrderProduct:   oq.withOrderProduct.Clone(),
...
// WithCarrierDrone tells the query-builder to eager-load the nodes that are connected to
// the "carrier_drone" edge. The optional arguments are used to configure the query builder of the edge.
func (oq *OrderQuery) WithCarrierDrone(opts ...func(*DroneQuery)) *OrderQuery {
	query := (&DroneClient{config: oq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	oq.withCarrierDrone = query
	return oq
}
...
// WithOrderWarehouse tells the query-builder to eager-load the nodes that are connected to
// the "order_warehouse" edge. The optional arguments are used to configure the query builder of the edge.
func (oq *OrderQuery) WithOrderWarehouse(opts ...func(*WarehouseQuery)) *OrderQuery {
	query := (&WarehouseClient{config: oq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	oq.withOrderWarehouse = query
	return oq
}
...
// WithOrderProduct tells the query-builder to eager-load the nodes that are connected to
// the "order_product" edge. The optional arguments are used to configure the query builder of the edge.
func (oq *OrderQuery) WithOrderProduct(opts ...func(*ProductQuery)) *OrderQuery {
	query := (&ProductClient{config: oq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	oq.withOrderProduct = query
	return oq
}
...
func (oq *OrderQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Order, error) {
	var (
		...
		_spec       = oq.querySpec()
		loadedTypes = [5]bool{
			oq.withCarrierDrone != nil,
			oq.withUserOrder != nil,
			oq.withOrderWarehouse != nil,
			oq.withOrderAddress != nil,
			oq.withOrderProduct != nil,
		}
	)
	if oq.withCarrierDrone != nil || oq.withUserOrder != nil || oq.withOrderWarehouse != nil || oq.withOrderAddress != nil || oq.withOrderProduct != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := oq.withCarrierDrone; query != nil {
		if err := oq.loadCarrierDrone(ctx, query, nodes, nil,
			func(n *Order, e *Drone) { n.Edges.CarrierDrone = e }); err != nil {
			return nil, err
		}
	}
	...
	if query := oq.withOrderWarehouse; query != nil {
		if err := oq.loadOrderWarehouse(ctx, query, nodes, nil,
			func(n *Order, e *Warehouse) { n.Edges.OrderWarehouse = e }); err != nil {
			return nil, err
		}
	}
	...
	if query := oq.withOrderProduct; query != nil {
		if err := oq.loadOrderProduct(ctx, query, nodes, nil,
			func(n *Order, e *Product) { n.Edges.OrderProduct = e }); err != nil {
			return nil, err
		}
	}
...
func (oq *OrderQuery) loadCarrierDrone(ctx context.Context, query *DroneQuery, nodes []*Order, init func(*Order), assign func(*Order, *Drone)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Order)
	for i := range nodes {
		if nodes[i].drone_current_order == nil {
			continue
		}
		fk := *nodes[i].drone_current_order
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "drone_current_order" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (oq *OrderQuery) loadUserOrder(ctx context.Context, query *UserQuery, nodes []*Order, init func(*Order), assign func(*Order, *User)) error {
	...
	nodeids := make(map[uuid.UUID][]*Order)
	for i := range nodes {
		if nodes[i].user_user_orders == nil {
			continue
		}
		fk := *nodes[i].user_user_orders
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_user_orders" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (oq *OrderQuery) loadOrderWarehouse(ctx context.Context, query *WarehouseQuery, nodes []*Order, init func(*Order), assign func(*Order, *Warehouse)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Order)
	for i := range nodes {
		if nodes[i].warehouse_warehouse_order == nil {
			continue
		}
		fk := *nodes[i].warehouse_warehouse_order
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "warehouse_warehouse_order" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (oq *OrderQuery) loadOrderAddress(ctx context.Context, query *AddressQuery, nodes []*Order, init func(*Order), assign func(*Order, *Address)) error {
	...
	nodeids := make(map[uuid.UUID][]*Order)
	for i := range nodes {
		if nodes[i].address_address_order == nil {
			continue
		}
		fk := *nodes[i].address_address_order
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "address_address_order" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (oq *OrderQuery) loadOrderProduct(ctx context.Context, query *ProductQuery, nodes []*Order, init func(*Order), assign func(*Order, *Product)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Order)
	for i := range nodes {
		if nodes[i].product_product_order == nil {
			continue
		}
		fk := *nodes[i].product_product_order
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "product_product_order" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
