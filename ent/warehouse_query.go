...
// QueryWarehouseOrder chains the current query on the "warehouse_order" edge.
func (wq *WarehouseQuery) QueryWarehouseOrder() *OrderQuery {
	query := (&OrderClient{config: wq.config}).Query()
...
func (wq *WarehouseQuery) Clone() *WarehouseQuery {
	...
	return &WarehouseQuery{
		...
		withWarehouseOrder: wq.withWarehouseOrder.Clone(),
...
// WithWarehouseOrder tells the query-builder to eager-load the nodes that are connected to
// the "warehouse_order" edge. The optional arguments are used to configure the query builder of the edge.
func (wq *WarehouseQuery) WithWarehouseOrder(opts ...func(*OrderQuery)) *WarehouseQuery {
	query := (&OrderClient{config: wq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	wq.withWarehouseOrder = query
	return wq
}
...
func (wq *WarehouseQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Warehouse, error) {
	...
	if query := wq.withWarehouseOrder; query != nil {
		if err := wq.loadWarehouseOrder(ctx, query, nodes, nil,
			func(n *Warehouse, e *Order) { n.Edges.WarehouseOrder = e }); err != nil {
			return nil, err
		}
	}
...
func (wq *WarehouseQuery) loadWarehouseOrder(ctx context.Context, query *OrderQuery, nodes []*Warehouse, init func(*Warehouse), assign func(*Warehouse, *Order)) error {
	...
	query.Where(predicate.Order(func(s *sql.Selector) {
		s.Where(sql.InValues(warehouse.WarehouseOrderColumn, fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.warehouse_warehouse_order
		if fk == nil {
			return fmt.Errorf(`foreign-key "warehouse_warehouse_order" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "warehouse_warehouse_order" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
